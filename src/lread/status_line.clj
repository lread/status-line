...
(defn line
  "Print a `msg` of `type`.
   ...
   `type` determines formatting and can be one of:
   - `:head` - used for headings
   - `:warn` - to draw the eye to something non-fatal but to be concerned about
   - `:error` - for things more dire
   - `:detail` - emits an unadorned msg

   `msg` can be multiline and include replacements for optional `fmt-args` ala clojure.format"
  [type msg & fmt-args]
  (let [msg (apply format msg fmt-args)]
    (println (case type
               :detail msg
               :head (format-info-line msg {:prefix "["
                                            :suffix "]"
